...
async def register(data: UserCreate, db: Session = Depends(get_db)):
    ...
    if not free_sub:
        raise HTTPException(500, "Abonnement Free manquant")
    user = User(
        user_full_name=data.user_full_name, user_email=data.user_email,
        user_subscription_id=free_sub.subscription_id, is_active=True,
        preferences={"language": "fr", "notifications": True},
    )
    db.add(user); db.flush()
    db.add(UserPassword(user_id=user.user_id, password_hash=get_password_hash(data.password)))
